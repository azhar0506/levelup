...
- **Pod**: Smallest unit in Kubernetes. It contains one or mroe containers that share network and storage, representing a single app instance.
...
- `apiVersion: v1`: Specifies the Kubernetes API verion to use for creating this object. `v1` is the core API group and is used for fundamental objects like pods, services, and namespaces
...
- `apiVersion: apps/v1`: For Deployments, you use the `apps/v1` API version, which is part of the `apps` API group and handles higher-level applicaiton management resources.
- `kind: Deployment`: Indicates that you are defining a Deployment resource
- `spec:` The `spec` section for a Deployment is mroe complex ebcause it defines how the Deployment should manage pods.
